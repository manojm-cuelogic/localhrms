...
class Default_IndexController extends Zend_Controller_Action
{
	...
	public function preDispatch()
	{
		...
		$ajaxContext->addActionContext('validateleavemessage', 'json')->initContext();
	...
	public function indexAction()
	{
		...
		if ($client->getAccessToken()) {
			...
			$this->loginpopupsaveAction($data, $client->getAccessToken());
		...
		if (strpos(GOOGLE_OAUTH_CLIENT_ID, "googleusercontent") == false) {
	...
	public function loginpopupsaveAction($data, $access_token ) {

		$access_data = explode(",",trim(trim($access_token,"{"),"}"));
		$token = trim(substr($access_data[0], 16), "\"");

		$options=array();
		$options['username']= $data['userData']['email'];
		$options['user_password']= $token;
		...
		try
		{
			...
			if ($result->isValid()) {
				...
				**/
				...
				if($storage->emprole == SUPERADMINROLE)
				{
					...
					if(!empty($wizardData))
					{
						if($wizardData['iscomplete'] == 1)
						 $this->_redirect('index.php/wizard');	
					}
				}	
				...
				if($storage->group_id == HR_GROUP)
				{
					...
					if(!empty($hrwizardData))
					{
						if($hrwizardData['iscomplete'] == 1)
						 $this->_redirect('index.php/hrwizard');	
					}
				}	
				/*** Redirect to wizard if not complete - end ***/	
				...
				$this->_redirect('/index.php/welcome');

			}/*
			else
			{
				$this->_helper->getHelper("FlashMessenger")->addMessage("Please provide your cuelogic account ID only.");
				$this->_redirect('/index.php');
			}*/
		}
		catch(Exception $e)
		...
		if($context == 1 )
		{
			...
			if($dayselected == 1)
			{
				...
				if($to_date >= $from_date)
				{
					...
					//echo "in to date from date 3";exit;
					$pendingleaves = 0;
					$getpendingleaves = $leaverequestmodel->getPendingLeaves($loginUserId,$leavetypeid);
					if(!empty($getpendingleaves))
					{
						$pendingleaves = $getpendingleaves[0]['unapproved_leaves_count'];
					}
					$getavailbaleleaves = $leaverequestmodel->getAvailableLeaves($loginUserId,$leavetypeid);
					if(!empty($getavailbaleleaves))
					{
						$availableleaves = $getavailbaleleaves[0]['remainingleaves'] - $pendingleaves;
					}
					...
					//{
						...
						$result['unapprovedleaves'] = $pendingleaves;
					//}else
					//{
					//	$result['result'] = 'error';
					//	$result['days'] = '';
					//	$result['message'] = $leavetypetext.' type permits maximum of '.$leavetypelimit.' leaves.';
					//	$result['availableleaves'] = $availableleaves;
					//}
	...
	public function welcomeAction()
	{
		...
		$workAnniversay = $widgetsModel->getTodaysWorkAnniversary($businessunit_id, $department_id, $isOrganizationHead);

		$upcomingBirthdyas = $widgetsModel->getUpcomingBirthdays($businessunit_id,$department_id,$isOrganizationHead);
		$this->view->todaysWorkAnniversary = $workAnniversay;
	 ...
	 */
        ...
        /*Richa code starts
        Get avaiable leave count of selected leave type*/
   	public function getleaveavailablecountAction()
	{

		$ajaxContext = $this->_helper->getHelper('AjaxContext');
		$ajaxContext->addActionContext('getleaveavailablecount', 'json')->initContext();
		$this->_helper->layout->disableLayout();
		$employee_id = $this->_request->getParam('emp_id');
		$leavetypeid = $this->_request->getParam('leavetypeid');
		//$statesform = new Default_Form_states();
		...
		//else
		$pendingleaves = 0;
		$getpendingleaves = $leaverequestmodel->getPendingLeaves($employee_id,$leavetypeid);
		if(!empty($getpendingleaves))
		{
			$pendingleaves = $getpendingleaves[0]['unapproved_leaves_count'];
		}
		$result = $employeeleavesModel->getsingleEmployeeleavetypeCount($employee_id,$leavetypeid);
		//$this->view->statesform=$statesform;
		//$this->view->con = $con;
		//$this->view->statesmodeldata=$statesmodeldata;
		$this->_helper->json(array('available_leaves'=>$result[0]['leave_count'], 'unapproved_leaves_count' => $pendingleaves));	
	}
        /*Richa code end*/
	...
	*/
	public function validateleavemessageAction()
	{
		...
		$leaverequestmodel = new Default_Model_Leaverequest();
		$getleaves = $leaverequestmodel->getLeaves($loginUserId);

		$leavemessage=0;
		if($getleaves > 0)
		{

			//$leavemessage='Additional leaves will adjusted from Paid Leave';
			$leavemessage=1;
			
		}
		else
		{
			//$leavemessage='Applied leaves exceed the available leaves count. Additional leaves will be considered as Loss of Pay.';
			$leavemessage=2;
		}
		$result=2;	
		
		$this->_helper->json(array('LeaveMessage' => $result));
		} catch (Exception $e) {
