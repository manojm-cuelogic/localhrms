...
class Default_LeaverequestController extends Zend_Controller_Action {
    ...
    public function indexAction() {
        ...
        if ($found) {
            ...
            if ($loginUserId != '' && $loginUserId != NULL) {
                ...
                //print_r($loggedInEmployeeDetails);exit;
            ...
            if (!empty($leavetype)) {
                if (sizeof($leavetype) > 0) {
                    foreach ($leavetype as $leavetyperes) {
                        ...
                        if ($leavetyperes['id'] == 1) {
                            $leaverequestform->leavetypeid->setValue($leavetyperes['id'] . '!@#' . $leavetyperes['numberofdays'] . '!@#' . utf8_encode($leavetyperes['leavetype']));
                        }
                        //$leaverequestform->setDefault('leavetypeid',$data['leavetypeid']);
                        //Richa code start for dynamic changge of available leaves on leave type change 
                        $leaverequestform->leavetypeid->setAttrib('onchange', 'employeeAvailableLeaves(' . $loginUserId . ',this)');
                    }
                }
            } else {
             ...
             */
            //Richa code start
            $leavetypeid = 1; //setting default value to be privilege leave
            $getunaprovedleavescount = 0;
            $getunaprovedleaves = $leaverequestmodel->getPendingLeaves($loginUserId, $leavetypeid);
            if (!empty($getunaprovedleaves)) {
                $getunaprovedleavescount = $getunaprovedleaves[0]['unapproved_leaves_count'];
                $leaverequestform->unapprovedleavesdaycount->setValue($getunaprovedleavescount);
            } else {
                $leaverequestform->unapprovedleavesdaycount->setValue($getunaprovedleavescount);
            }

            $getavailbaleleaves = $leaverequestmodel->getAvailableLeaves($loginUserId, $leavetypeid);
            if (!empty($getavailbaleleaves)) {
                ...
                $leaverequestform->no_of_days->setValue($leavesLeft);
            ...
            //echo "<pre>"; print_r($leaverequestform); exit;
            $this->view->form = $leaverequestform;
            $this->view->msgarray = $msgarray;
            $this->view->loginUserId = $loginUserIdAuth;
            $this->view->id = $id;
            $this->view->userfullname = $userfullname;
    ...
    public function saveleaverequestdetailsAction() {
        ...
        if ($loginUserId != '' && $loginUserId != NULL) {
            ...
            $loggedinEmpAuthId = $usersmodel->getUserDetailsByID($loginUserIdAuth);
            $loggedInEmployeeAuthDetails = $employeesmodel->getLoggedInEmployeeDetails($loginUserIdAuth);
        ...
        $leave_request_id = ""; //$this->_request->getParam('id'); 
        ...
        if (isset($leavetypeparam) && $leavetypeparam != '') {
            ...
            //$leavetypeArrTest = $employeeleavesModel->getsingleEmployeeleavetypeCount($id,$leavetypeid);
            //echo "Richa testing<pre>";
            //print_r($leavetypeArrTest);
        }
        ...
        if (!empty($leavetypeArr)) {
            $leavetypecount = $leavetypeArr[0]['numberofdays']; //emp_leave_limit
        ...
        if ($this->getRequest()->getPost()) {
            ...
            if ($leaverequestform->isValid($this->_request->getPost()) && $errorflag == 'true') {
                ...
                $data = array('user_id' => $loginUserId,
                    ...
                    'applied_by' => $loginUserId,
                    'approved_by' => ($id > 0 ? $loginUserId : NULL),
                ...
                if ($leave_request_id != '') {
                    $where = array('id=?' => $leave_request_id);
                    $actionflag = 2;
                } else {
               ...
               // Zend_Debug::dump($data);
                ...
                if ($Id == 'update') {
                    ...
                    if ($leavestatus == 2)
                        $updateemployeeleave = $leaverequestmodel->updateemployeeleaves($appliedleavescount, $loginUserId, $leavetypeid);
                    $tableid = $Id;
                    ...
                    if ($id > 0) {
                        ...
                        $options['message'] = '<div>Hi,</div><div>The below leave(s) has been applied and approved by ' . $loggedinEmpAuthId[0]['userfullname'] . ' on behalf of ' . $userfullname . '</div>';
                        $options['message'] .= '<div>
											                <table width="100%" cellspacing="0" cellpadding="15" border="0" style="border:3px solid #BBBBBB; font-size:16px; font-family:Arial, Helvetica, sans-serif; margin:30px 0 30px 0;" bgcolor="#ffffff">
											            	     ...
											            	     <tr>
											                  	...
											                  	<tr bgcolor="#e9f6fc">
											                        ...
											                        <td>' . $loggedinEmpAuthId[0]['userfullname'] . '</td>
											                  	</tr>
											                  	<tr bgcolor="#e9f6fc">
                        ...
                        $options['header'] = 'Leave Request';
                        $options['cc'] = array();
                        $options['toEmail'] = $loggedinEmpAuthId[0]['emailaddress'];
                        $options['toName'] = $loggedinEmpAuthId[0]['userfullname'];
                        $options['subject'] = 'Leave Request - ' . $userfullname . ' #' . $id;
                        $options['message'] = '<div>Hi,</div><div>The below leave(s) has been applied and approved by you on behalf of ' . $userfullname . '</div>';
                        $options['message'] .= '<div>
											                <table width="100%" cellspacing="0" cellpadding="15" border="0" style="border:3px solid #BBBBBB; font-size:16px; font-family:Arial, Helvetica, sans-serif; margin:30px 0 30px 0;" bgcolor="#ffffff">
											                      <tbody><tr>
											                        <td width="28%" style="border-right:2px solid #BBBBBB;">Employee Name</td>
                        ...
                        if (!empty($toemailArr)) {
                            ...
                            $options['subject'] = 'Leave Request - ' . $userfullname . ' #' . $tableid;
                            $options['header'] = 'Leave Request';
