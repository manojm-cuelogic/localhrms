...
class Default_EmpleavesController extends Zend_Controller_Action
{
		 	...
		 	try
		 	{
			    if($id && is_numeric($id) && $id>0)
				{
				  ...
				  if(!empty($empdata))
				  {
					...
					{
						//$result = $this->save($empleavesform,$id,$used_leaves,$leavetransfercount,$isleavetrasnferset);
						...
						if($id)
						{
						   ...
						   $empjoiningyear=date("y",$empjoiningdate);
							...
							{								
								...
								$leavetransferArr = $leavemanagementModel->getWeekendDetails($empdata[0]['department_id']);
								$prevyeardata = $employeeleavesModal->getPreviousYearEmployeeleaveData($id);
								/*Richa shifted code to save function */
								...
								$this->view->data = $data;
								$this->view->id = $id;
								$this->view->leavetype = $leavetypetext;
								$this->view->leavetransfercount = $leavetransfercount;
	...
	public function save($empleavesform,$userid,$used_leaves,$leavetransfercount,$isleavetrasnferset)
	{
		$auth = Zend_Auth::getInstance();
		if($auth->hasIdentity()){
			$loginUserId = $auth->getStorage()->read()->id;
		...
		if($empleavesform->isValid($this->_request->getPost())){
			...
			//code shifted from Edit action as per leave type 
			$employeeleavesModal = new Default_Model_Employeeleaves();
			$currentyeardata = $employeeleavesModal->getsingleEmployeeleaveData($userid);
			...
			{
				$currentdata = "notempty";
				for($i=0; $i<=sizeof($currentyeardata);$i++){
					if($currentyeardata[$i]['leavetypeid']==$leavetypeid){
						$currentyearleavecount = $currentyeardata[$i]['emp_leave_limit'];
						break;
					}
				}
			...
			/* END */
			$Id = $employeeleavesModel->SaveorUpdateEmployeeLeaves($user_id, $emp_leave_limit,$leavetypeid,$isleavetrasnfer,$loginUserId);
			if($id)
			{
	...
	public function viewpopupAction()
	{
		...
		$data = $employeeleavesModal->getsingleEmpleavesrowWithUsedLeaves($id);
		if(!empty($data))
