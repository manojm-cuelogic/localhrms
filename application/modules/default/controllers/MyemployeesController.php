...
class Default_MyemployeesController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
		...
		foreach($employeeModel->getEmployeeDept($loginUserId) as $dept) {
			$departmentId = $dept;
		}
		foreach($roleModel->getGroupIdByUserId($loginUserId) as $gp_id) {
			$groupId = $gp_id;
		}
		$reportingManagerData 	= $usersModel->getReportingManagerList_employees($departmentId,$loginUserId,$groupId, true);

		array_push($data,$dataTmp);
		$this->view->dataArray = $data;
		$this->view->reportingManagers = $reportingManagerData;
		$this->view->call = $call;
	...
	public function changemanagerpopupAction(){

		Zend_Layout::getMvcInstance()->setLayoutPath(APPLICATION_PATH."/layouts/scripts/popup/");

		$auth = Zend_Auth::getInstance();
		if($auth->hasIdentity()){
			$loginUserIdAuth = $loginUserId = $auth->getStorage()->read()->id;
		}
		$id = $this->getRequest()->getParam('id');
		$newRMid = $this->_request->getParam('newRM');
		$found = true;
		if($id > 0){ 
			$myemployees = new Default_Model_Myemployees(); 
			$found = false;
			foreach($myemployees->getTeamIds($loginUserIdAuth) as $ids) {
				if($ids['user_id'] == $id){
					$found = true;
					break;
				}
		...
		$employeeModel 			= new Default_Model_Employee();
		//$usersModel 			= new Default_Model_Users();
		//$departmentId 			= $employeeModel->getEmployeeDept($id);
		//$reportingManagerData 	= $usersModel->getReportingManagerList_employees($departmentId,'',$role_split[1]);
		if($employeeModel->changeReportingManager($id, $newRMid)) {
			//email functionality
			$employeeModel->changeReporteeLeaves($id, $newRMid, $loginUserId);
			$this->sendEMailAfterReporteeChange($id, $newRMid, $loginUserId);
	...
	public function addAction()
	{
		...
		{
			$emp_id = $emp_identity_code.str_pad($user_model->getMaxEmpId($emp_identity_code), 3, '0', STR_PAD_LEFT);
		}
		else
		{
			$emp_id = '';
		...
		if($empcommdetailsform->isValid($this->_request->getPost())){
           ...
           if(!empty($new_post_values))
           {
			...
			$data = array('user_id'=>$user_id,
								 ...
								 'emergency_number_1'=>($emergency_number!=''?$emergency_number:NULL),
								 'emergency_name_1'=>($emergency_name!=''?$emergency_name:NULL),
								 'emergency_email_1'=>($emergency_email!=''?$emergency_email:NULL),
								 'modifiedby'=>$loginUserId,
			                     'modifieddate'=>gmdate("Y-m-d H:i:s")			
			);
			if($id!=''){
	...
	function sendEMailAfterReporteeChange($id, $newRMid, $loginUserId, $pendingLeaves, $pendingLeaves) {
		$employeeModal = new Default_Model_Employee();
		$employeeDetail = $employeeModal->getsingleEmployeeData($id);
		$oldManagerDetail = $employeeModal->getsingleEmployeeData($loginUserId);
		$newManagerDetail = $employeeModal->getsingleEmployeeData($newRMid);

		/** Email to employee Start  */
		$options = array();
		$options['header'] = 'Reporting Manager Changed';
		$options['toEmail'] = $employeeDetail[0]['emailaddress'];
		$options['toName'] =  $employeeDetail[0]['userfullname'];
    	$options['subject'] = 'Your Reporting Manager has been changed';
		$options['message'] = '<div>Hi ' .$employeeDetail[0]['firstname']. ',</div><div>Your reporting manager has been changed from ' . $oldManagerDetail[0]['userfullname'] . ' to ' . $newManagerDetail[0]['userfullname'] . '. </div>';
		$options['message'] .= '
					            <div style="padding:20px 0 10px 0;">Your pending leaves have been forwarded to your new manager. Please <a href="'.BASE_URL.'" target="_blank" style="color:#b3512f;">click here</a> to login and check the leave details.</div>';	
		$result = sapp_Global::_sendEmail($options);

		/** Email to employee End  */

		/** Email to old manager Start  */

		$options = array();
		$options['header'] = 'Reporting Manager Changed';
		$options['toEmail'] = $oldManagerDetail[0]['emailaddress'];
		$options['toName'] = $oldManagerDetail[0]['userfullname'];
		$options['cc'] = array();
    	$options['subject'] = 'You have changed the Reporting Manager of ' . $employeeDetail[0]['userfullname'];
		$options['message'] = '<div>Hi ' .$oldManagerDetail[0]['firstname']. ',</div><div>You have changed the reporting manager of ' . $employeeDetail[0]['userfullname'] . ' to ' . $newManagerDetail[0]['userfullname'] . '.</div></br>';
		$options['message'] .= '
					            <div style="padding:20px 0 10px 0;"><b><u>Note:</u></b> Also the pending leaves of the employee have been forwarded to the new manager.</div>';	
		$result = sapp_Global::_sendEmail($options);
		
		/** Email to old manager End  */

		/** Email to new manager Start  */

		$options = array();
		$options['header'] = 'Reporting Manager Changed';
		$options['toEmail'] = $newManagerDetail[0]['emailaddress'];
		$options['toName'] = $newManagerDetail[0]['userfullname'];
		$cc_temp = EMAIL_HR_CC;
		$cc = explode(",", $cc_temp);
		$options['cc'] = $cc;
    	$options['subject'] = 'Reporting Manager changed';
		$options['message'] = '<div>Hi ' .$newManagerDetail[0]['firstname']. ',</div><div>You have been assigned as the new reporting manager of ' . $employeeDetail[0]['userfullname'] . '. Previously ' . $oldManagerDetail[0]['userfullname'] . ' was the manager of  ' . $employeeDetail[0]['userfullname'] . '.</div></br>';
		$options['message'] .= '
					            <div style="padding:20px 0 10px 0;">Also the pending leaves of the employee have been forwarded to you. Please <a href="'.BASE_URL.'" target="_blank" style="color:#b3512f;">click here</a> to login and check the leave details.</div>';	
		$result = sapp_Global::_sendEmail($options);
		
		/** Email to new manager End  */
	}
}