...
class Default_MydetailsController extends Zend_Controller_Action
{
	...
	public function otherdetailsAction()
	{
		if(defined('EMPTABCONFIGS'))
		{
			$empOrganizationTabs = explode(",",EMPTABCONFIGS);
			if(in_array('emp_others',$empOrganizationTabs)){
				//die("reach");
				$auth = Zend_Auth::getInstance();
				...
				if($callval == 'ajaxcall')
					$this->_helper->layout->disableLayout();
				$objName = 'others';$editPrivilege="";
				$employeeModel = new Default_Model_Employee;
				$emppersonaldetailsModel = new Default_Model_Emppersonaldetails();
				$emppersonaldetailsform = new Default_Form_empotherdetails();

				if(isset($_POST) && count($_POST) > 0 && $_POST['submit']) {
					$functional_area = $this->_request->getParam('functional_area');
					$project_status = $this->_request->getParam('project_status');
					$projects = $this->_request->getParam('project');
					$skype_id = $this->_request->getParam('skype_id');
					$prev_hr_email = $this->_request->getParam('prev_hr_email');
					$employeeModel->addOtherDetails($project_status, $functional_area);
					$employeeModel->addProjects($projects);
					$emppersonaldetailsModel->addOtherDetails($skype_id, $prev_hr_email);
				}


				$empdata = $employeeModel->getsingleEmployeeData($id); //print_r($empdata);exit;
				$emppersonaldata = $emppersonaldetailsModel->getsingleEmpPerDetailsData($id); //print_r($emppersonaldata);exit;

				$functional_area = explode(",", FUNCTIONAL_AREA); 
				foreach ($functional_area as $fa)
				{
					$emppersonaldetailsform->functional_area->addMultiOption($fa,$fa);
				}

				$projects = $emppersonaldetailsModel->getAllProjects();
				foreach ($projects as $key => $value) {
					$emppersonaldetailsform->project->addMultiOption($value['id'], $value['project_name']);
				}
				$my_projects = $emppersonaldetailsModel->getMyProjects($id);
				$def_my_projects = array();
				foreach ($my_projects as $key => $value) {
					$def_my_projects[] = $value['id'];
				}
				$emppersonaldetailsform->project->setValue($def_my_projects);
				$emppersonaldetailsform->functional_area->setValue($empdata[0]['functional_area']);
				$emppersonaldetailsform->project_status->setValue($empdata[0]['emp_project_status']);
				$emppersonaldetailsform->skype_id->setValue($emppersonaldata[0]['skype_id']);
				$emppersonaldetailsform->prev_hr_email->setValue($emppersonaldata[0]['prev_org_hr_email']);
				// if($empdata == 'norows')
				// {
				// 	$this->view->rowexist = "norows";
				// 	$this->view->empdata = "";
	...
	public function savepersonaldetails($emppersonaldetailsform,$userid,$identityDocumentArr)
	{
		...
		if($emppersonaldetailsform->isValid($this->_request->getPost()) && $errorflag == 'true'){
		 ...
		 }else
		 {
			...
			$this->_redirect('mydetails/personaldetailsview');
		}else
		{
				...
				try
				{
					...
					else
					{
						...
						if(!empty($empdata))
						{
							...
							//$usedleaves = $leaverequestmodel->employeeUsedLeaves($loginUserId,);
						...
						$tableFields = array('action'=>'Action','leavetypeid'=>'Leave Type','used_leaves'=>'Leaves Availed','remainingleaves'=>'Leave Balance','pending_leaves'=>'Pending Leaves','alloted_year'=>'Alloted Year');
		...
		if($employeeDetailsform->isValid($this->_request->getPost()))
		{
           ...
           {	
			...
			{	
				...
				case 'communication':
						...
						$emergency_number_1 = $this->_request->getParam('emergency_number_1');
						$emergency_name_1 = $this->_request->getParam('emergency_name_1');
						$relation_emergency_1 = $this->_request->getParam('relation_emergency_1');
						$emergency_number_2 = $this->_request->getParam('emergency_number_2');
						$emergency_name_2 = $this->_request->getParam('emergency_name_2');
						$relation_emergency_2 = $this->_request->getParam('relation_emergency_2');
						...
						$data = array('user_id'=>$user_id,
								 ...
								 'emergency_number_1'=>($emergency_number_1!=''?$emergency_number_1:NULL),
								 'emergency_name_1'=>($emergency_name_1!=''?$emergency_name_1:NULL),
								 'relation_emergency_1'=>($relation_emergency_1!=''?$relation_emergency_1:NULL),
								 'emergency_number_2'=>($emergency_number_2!=''?$emergency_number_2:NULL),
								 'emergency_name_2'=>($emergency_name_2!=''?$emergency_name_2:NULL),
								 'relation_emergency_2'=>($relation_emergency_2!=''?$relation_emergency_2:NULL),
								 'modifiedby'=>$loginUserId,
								 'modifieddate'=>gmdate("Y-m-d H:i:s")
						);
         ...
         }else
         {
			...
			if($tabName == "employee")	
				...
				$this->_redirect('mydetails/communicationdetailsview');	
			else
