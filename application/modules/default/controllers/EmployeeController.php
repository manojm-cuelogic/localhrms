		...
		try
		{
			if($id!='' && is_numeric($id) && $id>0 && $id!=$loginUserId)
			{
				...
				{  
						...
						$employee_types = $employeeModal->getAllEmployeeType();
						if($data == 'norows')
						{                                    
						...
						else if(!empty($data))
						{
						   ...
						   if(sizeof($businessunitData) > 0)
							...
							$employeeform->employee_type->addMultiOptions($employee_types);
...
public function editappraisal($id,$performanceflag,$ff_flag)
	...
	public function save($employeeform)
	{
		...
		$employee_type = $this->_request->getParam('employee_type',null);
		...
		if($employeeform->isValid($this->_request->getPost()) && $errorflag == 'true')
		{
			...
			$middlename = trim($this->_getParam('middlename',null));
			$lastname = trim($this->_getParam('lastname',null));
			$userfullname = $firstname . ((isset($middlename) && strlen(trim($middlename)) > 0) ? ' ' . $middlename : '') . ' '.$lastname;
		...
		else
		{
			...
			try
			{
				...
				else if($status == 'inactive')
				{
					...
					$empdata = array('emp_status_id' => 9,'isactive' => 0);
					$logarr = array('userid' => $loginUserId,
											'recordid' =>$emp_id,
					 			'date' => gmdate("Y-m-d H:i:s"),
                                             'isactive' => 0
	...
	public function changereportingmanagerAction()
	{
		...
		foreach($employessunderEmpId as $employee)
		{
			...
			$options['message'] = '<div>Hello '.ucfirst($employee['userfullname']).',
										...
										<div style="padding:20px 0 10px 0;">Please <a href="'.$baseUrl.'" target="_blank" style="color:#b3512f;">click here</a> to login </div>
									</div>';
			$result = sapp_Global::_sendEmail($options);
