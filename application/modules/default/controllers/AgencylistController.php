			...
			if(empty($websiteExistance))
			{
				...
				if($id=='')
				{
					...
					$options['message'] = 	'<div>Dear '.$pocfn.',</div>
											<div>'.ucfirst($agencyname).' agency has been created. The login credentials for your Cuelogic '.APPLICATION_NAME.' account is your cuelogic gmail account Id.
												<div></div>											
												<div style="padding:20px 0 10px 0;">Please <a href="'.$baseUrl.'/index" target="_blank" style="color:#b3512f;">click here</a> to login and check the agency details.</div>
											</div>';
					...
					foreach($emailids as $email)
					{
						...
						$options['message'] =	'<div>
													...
													<div></div>											
													<div style="padding:20px 0 10px 0;">Please <a href="'.$baseUrl.'" target="_blank" style="color:#b3512f;">click here</a> to login and check the agency details.</div>
												</div>';	
						$options['cron'] = 'yes';
						$result = sapp_Global::_sendEmail($options);
					}
					...
					$options['message'] = '<div>'.$agencynamemsg.'
											...
											<div style="padding:20px 0 10px 0;">Please <a href="'.$baseUrl.'" target="_blank" style="color:#b3512f;">click here</a> to login and check the agency details.
											</div>
											</div>';	
					$options['cron'] = 'yes';
					$result = sapp_Global::_sendEmail($options);
			...
			if($childIds)
			{
				...
				foreach($emailids as $email)
				{
					...
					$options['message'] = '<div>
												...
												<div></div>
												<div style="padding:20px 0 10px 0;">Please <a href="'.$custom_base_url.'" target="_blank" style="color:#b3512f;">click here</a> to login and check the agency details.</div>											
											</div>';	
					$options['cron'] = 'yes';
					$result = sapp_Global::_sendEmail($options);
				}
