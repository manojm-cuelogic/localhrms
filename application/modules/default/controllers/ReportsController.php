...
class Default_ReportsController extends Zend_Controller_Action
{
    ...
    **/
    ...
    public function getexcelleavebalanceAction()
    {
        require_once 'Classes/PHPExcel.php';
        require_once 'Classes/PHPExcel/IOFactory.php';
        $objPHPExcel = new PHPExcel();
        ...
        $letters = range('A','Z');
        $count =0;
        $filename = "LeaveBalanceReport.xlsx";

        // Make first row Headings bold and highlighted in Excel.
        $heading = array('Employee Id','Employee Name','PL Bal','SL Bal');
        foreach ($heading as $names)
        {
            $i = 1;
            $cell_name = $letters[$count].$i;
            $names = html_entity_decode($names,ENT_QUOTES,'UTF-8');
        ...
        $leaveData = array();
        foreach($leaveModel->getEmployeeLeaveBalance(1) as $val) {
            $leaveData[$val['user_id']]['name'] = $val['userfullname'];
            $leaveData[$val['user_id']]['pl']       = $val['leave_count'];
            $leaveData[$val['user_id']]['emp_id']   = $val['employeeId'];
        }
        foreach($leaveModel->getEmployeeLeaveBalance(2) as $val) {
            $leaveData[$val['user_id']]['sl']       = $val['leave_count'];
        }
        
        $i = 2;
        foreach($leaveData as $id=>$leave_data)
        {
            $count1 =0;
            
            $cell_name = $letters[$count1++].$i;
            $value = html_entity_decode($leave_data['emp_id'],ENT_QUOTES,'UTF-8');
            $objPHPExcel->getActiveSheet()->SetCellValue($cell_name, $value);

            $cell_name = $letters[$count1++].$i;
            $value = html_entity_decode($leave_data['name'],ENT_QUOTES,'UTF-8');
            $objPHPExcel->getActiveSheet()->SetCellValue($cell_name, $value);

            $cell_name = $letters[$count1++].$i;
            $value = html_entity_decode($leave_data['pl'],ENT_QUOTES,'UTF-8');
            $objPHPExcel->getActiveSheet()->SetCellValue($cell_name, $value);

            $cell_name = $letters[$count1++].$i;
            $value = html_entity_decode($leave_data['sl'],ENT_QUOTES,'UTF-8');
            $objPHPExcel->getActiveSheet()->SetCellValue($cell_name, $value);
    ...
    public function exportemployeereportAction()
    {
        ...
        $cols_param_arr = array(
                            ...
                            "emp_project_status" => "Project Status",
                            ...
                            "previous_experiance" => "Previous Exp(Years)",
                            "exp_in_curr" => "Exp. in Cuelogic",
                            "total_experiance" => "Total Exp",
                            "salary" => "Current Salary",
                            "emp_status_name" => "Employment Status",
                            "resignedpip_start_date" => "Resigned / PIP Start Date",
                            "resignedpip_last_date" => "Resigned / PIP Last Date",
                            "dob" => "DOB",
                            "genderid" => "Gender",
                            "skype_id" => "Skype ID",
                            "emailaddress" => "Official Email ID",
                            "personalemail" => "Personal Email",
                            "education" => "Educational Qualification",
                            "current_streetaddress" => "Current Address",
                            "perm_streetaddress" => "Permanent Address",
                            ...
                            "emergency_name_1" => "Emergency Person 1",
                            "emergency_number_1" => "Emergency Number 1",
                            "relation_emergency_1" => "Relation with Emergency Contact I",
                            "emergency_name_2" => "Emergency Person 2",
                            "emergency_number_2" => "Emergency Number 1",
                            "relation_emergency_2" => "Relation with Emergency Contact II",
                            "maritalstatusname" => "Marital Status",
                            "prev_org_hr_email" => "Prev Org HR ID"
                        );
        // Make first row Headings bold and highlighted in Excel.
        foreach ($cols_param_arr as $names)
        {
        ...
        foreach($emp_arr as $emp_data)
        {
            ...
            foreach ($cols_param_arr as $column_key => $column_name)
            {
                ...
                {
                    $value = isset($emp_data['userfullname'])?$emp_data['userfullname']:"";
                }
                ...
                elseif($column_key == 'previous_experiance')
                {
                    $value = "";
                    if(isset($emp_data['experiance']) && strlen(trim($emp_data['experiance'])) > 0) {
                        $total_expArr = explode(",", $emp_data['experiance']);
                        $days = 0;
                        for($x = 0; $x < count($total_expArr); $x += 2) {
                            $date1=date_create($total_expArr[$x]);
                            $date2=date_create($total_expArr[$x + 1]);
                            $diff=date_diff($date1,$date2);
                            $days += $diff->format("%a");
                        }
                        $value = $prev_exp = round(($days/365),1,PHP_ROUND_HALF_UP);
                    }
                }
                elseif($column_key == 'exp_in_curr')
                {
                    $value = "";
                    if(isset($emp_data['date_of_joining']) && strlen(trim($emp_data['date_of_joining'])) > 0) {
                        $days = 0;
                        $date1=date_create($emp_data['date_of_joining']);
                        $date2=date_create(date("Y-m-d"));
                        $diff=date_diff($date1, $date2);
                        $days = $diff->format("%a");
                        $value = $curr_exp = round(($days/365),1,PHP_ROUND_HALF_UP);
                    }
                }
                elseif($column_key == 'total_experiance')
                {
                    $value = "";
                    if($prev_exp != "") $total_exp += $prev_exp;
                    if($curr_exp != "") $total_exp += $curr_exp;
                    $value = $total_exp;
    ...
    public function leavesreportAction()
    {
        ...
        if($from_date !='' && $to_date != '')
        {
            ...
            $searchQuery .= 'DATE(l.from_date) >= "'.$from_date.'" AND DATE(l.to_date) <= "'.$to_date.'" AND ';
        } else if($from_date !='') {
            $from_date = sapp_Global::change_date($from_date,'database');
    ...
    public function getexcelreportleavesAction()
    {
        ...
        $searchQuery = '';
        $reportsmodel = new Default_Model_Reports();
        $emp_leave_model = new Default_Model_Employeeleaves();

        $cols_param = explode(',',$param_arr['checkedheaders']);
        $leavesheaderarr = array(
                            'reportingmanagername'=>'Reporting Manager',
                            'employeeId'=>'Employee ID',
                            ...
                            'sl'=>'SL consumed',
                            'reason'=>'Reason',
                            'pl_bal'=>'Current PL BAL',
                            'sl_bal'=>'Current SL BAL',
                            'total_bal'=>'Current Total Balance',
                            'planned'=>'Planned/Unplanned'
                            );
        $cols_param = array_values($leavesheaderarr);
        $cols_param_arr = array();
        foreach($leavesheaderarr as $key=>$val)
        {
            foreach( $cols_param  as $col)
        ...
        $plsl_data = $emp_leave_model->getEmployeeWiseLeaveBalance();
        //$cols_param_arr = array_values($leavesheaderarr);
        ...
        $searchQuery = rtrim($searchQuery," AND");
        $leavestatusArr = $reportsmodel->getEmpLeaveHistoryForExport($sort_name, $sort_type,$searchQuery);
        ...
        foreach($emp_arr as $emp_data)
        {
            ...
            foreach ($cols_param_arr as $column_key => $column_name)
            {
                ...
                else if($column_key == 'employeeId')
                {
                    $value = isset($emp_data['employeeId'])?$emp_data['employeeId']:"";
                }
                else if($column_key == 'pl')
                {
                    $value = ($emp_data['pl'] == 0 ? "" : $emp_data['pl']);
                }
                else if($column_key == 'sl')
                {
                    $value = ($emp_data['sl'] == 0 ? "" : $emp_data['sl']);
                }
                else if($column_key == 'pl_bal')
                {
                    $value = $plsl_data[$emp_data['id']]['pl'];
                }
                else if($column_key == 'sl_bal')
                {
                    $value = $plsl_data[$emp_data['id']]['sl'];
                }
                else if($column_key == 'total_bal')
                {
                    $value = $plsl_data[$emp_data['id']]['pl'] + $plsl_data[$emp_data['id']]['sl'];
                }
                else if($column_key == 'planned')
                {
                    $value = ($emp_data['pl'] == 0 ? "Unplanned" : "Planned");
                    //isset($emp_data['buss_unit_name'])?$emp_data['buss_unit_name'].$emp_data['department_name']:$emp_data['department_name'];
                }
                else if($column_key == 'reportingmanagername')
                {
                    $value = isset($emp_data['rep_manager_name'])?$emp_data['rep_manager_name']:"";
                }
                else if($column_key == 'from_date')
                {
                    $value = isset($emp_data['from_date'])?date("d M Y", strtotime($emp_data['from_date'])):"";
                }
                else if($column_key == 'to_date')
                {
                    $value = isset($emp_data['to_date'])?date("d M Y", strtotime($emp_data['to_date'])):"";
                }
                else
                {
                    $value = isset($emp_data[$column_key])?$emp_data[$column_key]:"";
