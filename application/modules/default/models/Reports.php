...
class Default_Model_Reports extends Zend_Db_Table_Abstract
{
    ...
    public function getEmpLeaveHistoryForExport($sort, $by, $searchQuery='')
    {
        ...
        $query = "SELECT l.rep_manager_name, mu.employeeId, mu.id, l.user_name,  l.from_date, l.to_date, if(l.leavetypeid = 1, l.appliedleavescount, 0) as pl, if(l.leavetypeid = 2, l.appliedleavescount, 0) as sl, reason FROM main_leaverequest_summary AS l left join main_users mu on mu.id = l.user_id where $where order by $sort $by";
  ...
  //                                        'leaveday'=>'if(l.leaveday = 1,"Full Day","Half Day")',                                          
        ...
        //echo "<pre>"; print_r($this->fetchAll($query)); exit;
