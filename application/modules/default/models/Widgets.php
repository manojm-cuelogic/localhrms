...
class Default_Model_Widgets extends Zend_Db_Table_Abstract
{
	...
	public function getTodaysBirthdays($businessunit_id,$department_id,$isOrganizationHead='')
	{
		...
		$qryStr = "select id,firstname, lastname, userfullname from main_users where id in (Select p.user_id from main_emppersonaldetails p inner join main_employees e on p.user_id = e.user_id  where day(now()) = day(p.dob) and month(now()) = month(p.dob) ".$whereStr.")";
	...
	public function getTodaysWorkAnniversary($businessunit_id,$department_id,$isOrganizationHead='')
	{
		$db = Zend_Db_Table::getDefaultAdapter();
		$whereStr = '';
		$qryStr = "select id,firstname, lastname, userfullname from main_users where id in (Select e.user_id from main_employees e where day(now()) = day(e.date_of_joining) and month(now()) = month(e.date_of_joining)  and year(now()) != year(e.date_of_joining)  ".$whereStr.") AND isactive = 1";
