...
class Default_Model_Leaverequest extends Zend_Db_Table_Abstract {
    ...
    /*
     * CREATED BY Manoj
     * DATED On 5th Nov 2016
     * @parameter $loginUserId int
     * PURPOSE = To check for Leave balance avialable for that particular Users
     * @return Integer
     */
    ...
    /*
     * CREATED BY Manoj
     * DATED On 9th Nov 2016 
     * PURPOSE = To get data from table having details less than today's date
     * @return array
     * 
     */

    public function getdetailspeandinlevesCron() {
    

        $select = $this->select();
  
        $select->setIntegrityCheck(false)
                ->from('main_leaverequest', 'main_leaverequest.*')
                ->joinInner('main_users', 'main_users.id=main_leaverequest.rep_mang_id', array("concat(main_users.firstname,' ',main_users.lastname) as
                rep_name",'main_users.emailaddress as rep_mang_email'))
                ->joinInner(array('empl' => 'main_users'), 'empl.id=main_leaverequest.user_id', array("concat(empl.firstname,' ',empl.lastname) as
                emp_name",'empl.emailaddress as emp_email'))
                    ->joinInner(array('main_employeeleavetypes'), 'main_employeeleavetypes.id=main_leaverequest.leavetypeid','main_employeeleavetypes.leavetype')
                ->where('DATE(main_leaverequest.to_date) < date(NOW())'
                        . 'and  main_leaverequest.leavestatus = 1 ');
    ...
    public function getLeaveStatusHistory($sort, $by, $pageNo, $perPage, $searchQuery, $queryflag = '', $loggedinuser, $managerstring = '') { //die($by);
        ...
        if ($by == 'leave_from_date')
            $by = 'from_date';
        else if ($by == 'leave_to_date')
            $by = 'to_date';
        else if ($by == 'leave_applieddate')
            $by = 'l.createddate';
        ...
        $leaveStatusData = $this->select()
                ->setIntegrityCheck(false)
                ->from(array('l' => 'main_leaverequest'), array('l.*', 'leave_from_date' => 'DATE_FORMAT(l.from_date,"%d %b %Y")',
    ...
    public function getEmployeeLeaveRequest($sort, $by, $pageNo, $perPage, $searchQuery, $loginUserId) {
        ...
        $employeeleaveData = $this->select()
                ->setIntegrityCheck(false)
                ->from(array('l' => 'main_leaverequest'), array('l.*', 'leave_from_date' => 'DATE_FORMAT(l.from_date,"' . DATEFORMAT_MYSQL . '")',
