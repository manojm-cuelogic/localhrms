...
class Default_Model_Users extends Zend_Db_Table_Abstract
{
    ...
    public function getUsersWithPicture()
    {
        ...
        $usersData = $db->query("select u.id,u.userfullname, u.profileimg from main_users u where (u.userstatus='new' OR u.userstatus='old') AND u.isactive=1 ");
        $usersResult = $usersData->fetchAll();
        return $usersResult;
    }
    ...
    public function getReportingManagerList($dept_id,$employee_id='',$employee_group='')
    {
         ...
         */
        public function getReportingManagerList_employees($dept_id ='',$employee_id='',$employee_group, $only_managers = false)
    {       
            /*
                When there are no managers for selected department.get management as reporting managers.
            */ //echo MANAGEMENT_GROUP;die($employee_group);
            $res="";$qry="";
            $db = Zend_Db_Table::getDefaultAdapter();
            if($employee_group != "")
            {
                ...
                else 
                {
                    if($dept_id != '')
                    {
                        ...
                        if($only_managers) {
                            $qry .= " AND u.emprole IN (2,3,8,14) ";
                            //echo $qry; exit;
                        }
                        else if(isset($employee_group) && !empty($employee_group) && $employee_group == MANAGEMENT_GROUP)
                        {
                                $qry .= " AND r.group_id IN (".MANAGEMENT_GROUP.") ";
                        }
                        else
                        ...
                        $qry .= "   r.group_id IN (".MANAGEMENT_GROUP ."," .MANAGER_GROUP .") AND u.isactive=1 AND r.isactive=1)";
    ...
    public function getUserNameImageByIds($userIdArray)
    {
        $resultstring = implode(',', $userIdArray);
        ...
        if($resultstring)
        {
             $qry = "select ob.id, ob.profileimg , ob.userfullname from main_users ob
                                        where ob.id IN (".$resultstring.") and ob.isactive = 1";        
                $db = Zend_Db_Table::getDefaultAdapter();
                $sqlRes = $db->query($qry);
                $userRes = $sqlRes->fetchAll();
                if(!empty($userRes))
                {
                    foreach($userRes as $user)
                    {
                        $userArray[$user['id']]= array('name' => $user['userfullname'], 'image' => $user['profileimg']);
                    }
                }
        }
        return $userArray;
    ...
    public function getUsersNotInIdsArray($userIdArray)
    {
        $db = Zend_Db_Table::getDefaultAdapter();
        $resultstring = implode(',', $userIdArray);
        $resultstring = trim($resultstring , ',');
        $userArray = array();
        $qry = "select ob.user_id, ob.profileimg , ob.userfullname, ob.reporting_manager from main_employees_summary ob where  ob.isactive = 1";  
        if($resultstring)
        {
           $qry .= " and ob.user_id NOT IN (".$resultstring.") ";    
        }
        $qry .= " order by ob.userfullname desc";
        $sqlRes = $db->query($qry);
        $userRes = $sqlRes->fetchAll();
        if(!empty($userRes))
        {
            foreach($userRes as $user)
            {
                $userArray[]= array( 'id' => $user['user_id'] , 'name' => $user['userfullname'], 'reporting_manager' => $user['reporting_manager'], 'image' => $user['profileimg']);
            }
        }
        return $userArray;
    }
 ...
 ELSE CASE WHEN u.id IN (SELECT reporting_manager FROM main_employees_summary) THEN 'Manager'
    ...
    /*Richa code start
    Defining function to get active users id*/
    public function getActiveUsersId()
    {           
            
                $result =  $this->select()
                ->setIntegrityCheck(false)
                ->from(array('u'=>'main_users'),array('u.id'))
                ->where("u.isactive = 1");
    ...
    /**
    * @author Aju John
    * This function will get the user details from the list of emplyee ids
    */
    public function getUserModelsFromEmployeeIds ($ids) {
        $condition = " and u.employeeId in (";
        foreach($ids as $k=>$id){
            if($k != 0)
                $condition .= ",";
            $condition .= "'".$id."'";
        ...
        $results =  $this->select()
                        ->setIntegrityCheck(false)
                        ->from(array('u'=>'main_users'))
                        ->where("u.isactive = 1 ".$condition);
        return $this->fetchAll($results)->toArray();
    }
    /**
    * @author Aju John
    * This function will get the user detials from employee name
    */
    public function getUserModelFromEmployeeName ($name) {
        $condition = " and u.userfullname = '".$name."'";
    ...
    public function getAllParents($user_id) {
        $loop = true;
        $parents = array();
        $index = 0;
        $db = Zend_Db_Table::getDefaultAdapter();
        while($loop) {
            $qry = "select user_id, userfullname, reporting_manager from main_employees_summary where user_id = " . $user_id; 
            $res = $db->query($qry)->fetchAll();
            if($res[0]['reporting_manager'] == 0) {
                $loop = false;
                if($index > 0)
                    $parents[$index-1] =  $res[0];
            } else {
                if($index > 0)
                    $parents[$index-1] =  $res[0];
                $user_id = $res[0]['reporting_manager'];
            }
            $index++;
        }
        return $parents;
    ...
    public function getReportingManagers($userId) {
    ...
    public function getImmediateManager($userId) {

        $db = Zend_Db_Table::getDefaultAdapter();
        $qryStr = 'select user_id, jobtitle_name as jobTitle, userfullname as userName, emailaddress as email, ( select profileimg as profileImage from main_users where  employeeId IN ( select reporting_manager from main_employees_summary where user_id="'.$userId.'") ) as profileImage
            from main_employees_summary where user_id= (select reporting_manager from main_employees_summary where
            user_id="'.$userId.'")';
        $qry = $db->query($qryStr);
        $res = $qry->fetchAll();
        return $res;
