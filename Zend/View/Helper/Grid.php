...
class Zend_View_Helper_Grid extends Zend_View_Helper_Abstract {
	...
	public function grid ($dataArray)
	{
				...
				if(!in_array('view',$actions_arr) && !in_array('edit',$actions_arr) && !in_array('delete',$actions_arr))
				{
				  ...
				  {	
					 ...
					 $extra['action'] = array('name' => 'edit', 'value' =>'<div class="grid-action-align">
								...
								<a onclick="displaydeptform(\''.BASE_URL.$dataArray['objectname'].'/'.$editaction.'/id/{{id}}'.$con.'/popup/1\',\''.$menunamestr.'\')" name="{{id}}" id="edit{{id}}" class="sprite edit"  title=\'Edit\' ></a>
		...
		else
		{
			$formgridVal = '';
						...
						if($dataArray['objectname'] == 'pendingleaves' || $dataArray['objectname'] =='approvedleaves')
						   $delete_str = '<a name="{{id}}"  id="cancel_{{id}}"onclick= changestatus(\''.$dataArray['objectname'].'\',\'{{id}}\',\''.$msgdta.'\')	href= javascript:void(0) title=\'Cancel Leave\' class="sprite cancel-lev" ></a>';
						...
						else if($dataArray['objectname'] == 'appraisalcategory' || $dataArray['objectname'] == 'appraisalquestions' || $dataArray['objectname'] == 'feedforwardquestions' || $dataArray['objectname'] == 'announcements')
						{
                        	...
                        	$delete_str = '<a name="{{id}}" onclick= changestatus(\''.$dataArray['objectname'].'\',\'{{id}}\',\''.$msgdta.'\')	href= javascript:void(0) title=\'Delete\' class="sprite delete" ></a>';
							$apply_leave = '<a name="{{id}}" href= "'.BASE_URL.'leaverequest/index/id/{{id}}"	href= javascript:void(0) title=\'Apply Leave\' class="sprite leave-apply" ></a>';
							$manager_change = '<a onclick="changeManager(\''.BASE_URL.$dataArray['objectname'].'/changemanagerpopup\',\'{{id}}\')" name="{{id}}"	href= javascript:void(0) title=\'Change Reporting Manager\' class="sprite manager-change" ></a>';
						...
						}else
						{
						  $extra['action'] = array('name' => 'edit', 'value' =>'<div class="grid-action-align">
										...
										'.((in_array('edit',$actions_arr)?$edit_str:'')).'
										'.((in_array('delete',$actions_arr)?$delete_str:'')).'
										'.((($dataArray['objectname'] == 'myemployees')?$apply_leave:'')).'
										'.((($dataArray['objectname'] == 'myemployees')?$manager_change:'')).'
	...
	public function generateGrid ($name, $fields = null,$paginator=null,$extracolumn=array(),$sorting=false,$jsGridFnname='', $perPage='5',$page='1', $jsFillFnName='',$searchArray='',$formgrid='false',$addaction='',$menuName='',$unitId,$addpermission,$menunamestr,$call='',$sortStr='',$search_filters = '',$dashboardCall = 'No',$empstatus = '',$actnArr,$empscreenTotalCount = '',$sortname='',$by='') {
		...
		foreach($paginator as $p) {
			...
			if(isset($p['leavestatus']) && trim($p['leavestatus'])=='Pending for cancellation'){
				$cell_color .= ' pendingleavecancellation'; 
			...
			if(isset($p['leavestatus']) && trim($p['leavestatus'])!='Pending for cancellation' && trim($p['leavestatus'])!='Pending for approval'){
				//$cell_color .= ' pendingleavecancellation'; 
				echo "<script type='text/javascript'>
											$(document).ready(function() { 
											$('#edit'+".$p['id'].").remove();
