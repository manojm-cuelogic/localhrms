		...
		*** commented to remove menu icon 07-08-2015 - END - ***/
		...
		{		
                    ...
                    if($menuid == SITEPREFERENCE)
                    {
                      ...
                      $hrTabAccessArr=[1,8,14,19];
                    if($data['menuName']=='HR' && !in_array($userRole, $hrTabAccessArr)){
                		$data['menuName']='Employees List';
                	}
                    if($con == 'settings')
                    {
		...
		//hack: temp fix to add OKR menu
		$parent_menu .= "<li id='main_parent_okr' super-parent = 'main_parent_' class = '".$clas_drag."clickable_menu ".$parent_menu_selected_class[DASHBOARD_MENU]."-main ".$tour_menu_class[DASHBOARD_MENU]."' menu-url ='".BASE_URL."welcome' selected-class = '".$parent_menu_selected_class[DASHBOARD_MENU]."' > <a id='okr'><b>OKR</b></a></li>";
