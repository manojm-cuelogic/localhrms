...
class Zend_View_Helper_Employeedetails extends Zend_View_Helper_Abstract {
	...
	public  function employeedetails($emparr,$conText,$userId)
	{
		...
		**/
		...
		$empdata .=	'<p><b>Email Id <i>:</i></b><span>'.$emparr['emailaddress'].'</span></p>';
	...
	public  function employeetabs($conText,$userId)
	{
		...
		else if($conText == "view")
		{
			if($group_id == HR_GROUP ||$group_id == MANAGEMENT_GROUP || $loggedinuser == SUPERADMIN)
			{
				...
				if(!empty($empOrganizationTabs) && in_array("emp_others", $empOrganizationTabs))
				$tabsHtml .= '<li id = "emp_others" onclick="changeempviewscreen(\'emp_others\','.$userId .');">'.TAB_EMP_OTHERS.'</li>';
		...
		else if($conText == "mydetails")
		{
			...
			if(!empty($empOrganizationTabs) && in_array("emp_others", $empOrganizationTabs))
				$tabsHtml .= '<li id = "emp_others"><a href="'.BASE_URL.'mydetails/otherdetails">'.TAB_EMP_OTHERS.'</a></li>';
