				...
				***/
				...
				if($('.tour_dashboard').attr('class'))
				{
					var obj = {
						...
						"color"		: "white",
						"position"	: "TL",
						"text"		: 'Cuelogic HRMS Dashboard is a real time user interface allowing you to have all the menus that are used regularly, at one place. The easy to read display of the menus covers overall information present in each menu with clickable sub menus to navigate to the desired page avoiding tiresome menu redirections. The display of dandy default features, birthdays and announcements, takes Cuelogic HRMS to another level altogether.' ,
						"time" 		: 5000
						};
					configArr.push(obj);
				}
					...
					switch(tip_position){
						...
						case 'R'	:
							...
							properties = {
								...
								'top'	: e_t + e_h/2 + 5 - $tooltip.height()/2 + 'px'
							};
						...
						case 'L'	:
							...
							properties = {//e_l + e_w + 'px'
								'left'	: e_l + e_w + 15 + 'px',//e_w + 13 + 'px', 
								'top'	: e_t + e_h/2 + 6 - $tooltip.height()/2 + 'px'
							};
