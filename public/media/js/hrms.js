$(document).ready(function () {
    ...
    setTimeout(function () {
        $(".three").fadeIn('slow');
        $(".login").fadeIn('slow');
        $(".bg_new .logins").fadeIn('slow');
    }, 1000)

    $("#hrm_feature").click(function () {
        $(".two").hide();
        $("#hrm_feature").hide();
        $("#abouttab").hide();
        $("#close_btn_feature").show();
        $(".feature-img").show();

        setTimeout(function () {
            $("#feature-one").addClass('feature-appear')
        }, 1000);
        setTimeout(function () {
            $("#feature-two").addClass('feature-appear')
        }, 1200);
        setTimeout(function () {
            $("#feature-three").addClass('feature-appear')
        }, 1400);
        setTimeout(function () {
            $("#feature-four").addClass('feature-appear')
        }, 2200);
        setTimeout(function () {
            $("#feature-five").addClass('feature-appear')
        }, 1600);
        setTimeout(function () {
            $("#feature-six").addClass('feature-appear')
        }, 2000);
        setTimeout(function () {
            $("#feature-seven").addClass('feature-appear')
        }, 1800);
    });

    $("#close_btn_feature").click(function () {
        $(".feature-img").hide();
        $(".two").show();
        $("#hrm_feature").show();
        $("#abouttab").show();
        $("#close_btn_feature").hide();
        $(".feature-img").removeClass('feature-appear')
    });
...
var configurationsArr = new Array('employmentstatus', 'eeoccategory', 'jobtitles', 'payfrequency', 'remunerationbasis', 'positions', 'bankaccounttype', 'competencylevel', 'educationlevelcode', 'attendancestatuscode', 'workeligibilitydoctypes', 'employeeleavetypes', 'ethniccode', 'timezone', 'weekdays', 'monthslist', 'gender', 'maritalstatus', 'prefix', 'racecode', 'nationalitycontextcode', 'nationality', 'accountclasstype', 'licensetype', 'numberformats', 'identitycodes', 'emailcontacts', 'countries', 'states', 'cities', 'geographygroup', 'veteranstatus', 'militaryservice', 'currency', 'currencyconverter', 'language', 'changemanager');
...
function getsearchdata(objname, conText, colname, event, etype)
{
    ...
    if ($("#columnId").length)
        ...
        $('#footer').append('<input type="hidden" value="' + columnid + '" id="columnId" />');
...
function fieldBlurvalidations(injury_typeVal)
{
    if (injury_typeVal == 3)
    {
        ...
        $('#s2id_disability_type').on("blur", function ()
        {
            ...
            {
                if ($('#errors-disability_type').length)
            ...
            } else if ($.trim($("#disability_type").val()) != '' && $.trim($("#disability_type").val()) == 'other impairments')
            {
                ...
                $('#other_disability_type').on("blur", function ()
                {
                    ...
                    {
                        if ($('#errors-other_disability_type').length)
    ...
    } else if (injury_typeVal == 4)
    {
        ...
        {
            if ($('#errors-injury_severity').length)
                $("#errors-injury_severity").remove();
            if ($.trim($(this).val()) == '')
            {
        ...
        {
            if ($('#errors-empleave_from_date').length)
                $("#errors-empleave_from_date").remove();
            if ($.trim($(this).val()) == '')
            {
        ...
        {
            if ($('#errors-empleave_to_date').length)
                $("#errors-empleave_to_date").remove();
            if ($.trim($(this).val()) == '')
            {
        ...
        {
            if ($('#errors-hospital_name').length)
                $("#errors-hospital_name").remove();
            if ($.trim($(this).val()) == '')
            {
        ...
        {
            if ($('#errors-hospital_addr').length)
                $("#errors-hospital_addr").remove();
            if ($.trim($(this).val()) == '')
            {
        ...
        {
            if ($('#errors-room_num').length)
                $("#errors-room_num").remove();
            if ($.trim($(this).val()) == '')
            {
        ...
        {
            if ($('#errors-gp_name').length)
                $("#errors-gp_name").remove();
            if ($.trim($(this).val()) == '')
            {
        ...
        {
            if ($('#errors-treatment_details').length)
                $("#errors-treatment_details").remove();
            if ($.trim($(this).val()) == '')
            {
        ...
        {
            if ($('#errors-total_cost').length)
                $("#errors-total_cost").remove();
            if ($.trim($(this).val()) == '')
            {
        ...
        $.post(base_url + "/index/fromdatetodate", {from_val: from_val, to_val: to_val, con: conText}, function (data) {
            ...
            } else if (data.result == 'yes')
            {
                $.ajax({
                    ...
                    {
                        if (conText == 1)
                            $("#" + from_date_id).before("<div id='loader'></div>");
                        else
                            $("#" + to_date_id).before("<div id='loader'></div>");
...
function displayempstatusmessage()
{
    ...
    if (empstatusval == 9 || empstatusval == 10)
    {
...
function downloadLeaveBalanceExcel(url) {
    $.blockUI({width: '50px', message: $("#spinner").html()});
    $.ajax({
        type: "POST",
        url: url,
...
/*Richa code start-
 function to get available leaves for selected leave type of employee
 */
function employeeAvailableLeaves(empid, leavetype) {
    var leavetypeArr = leavetype.value.split("!@#"),
    ...
    if (empid != '' && leavetypeid != '')
    {
        ...
        $.ajax({
            url: base_url + "/index/getleaveavailablecount/format/json",
            type: 'POST',
            data: 'emp_id=' + empid + '&leavetypeid=' + leavetypeid,
            dataType: 'json',
            success: function (response) {

                if (response['available_leaves'] != '' && response['available_leaves'] != 'null')
                {
