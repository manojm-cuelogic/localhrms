...
if( file_exists( $filepath ) ){
	...
	try{
		...
		require_once 'google-api/src/Google/Config.php';
		require_once 'google-api/src/Google/Client.php';
		require_once 'google-api/src/Google/Service.php';

		if(isset($_SERVER['HTTP_HOST']) && $_SERVER['HTTP_HOST'] !== "hrms.cuelogic.co.in") {
			require_once 'public/db_constants_local.php';
			require_once 'public/google_constants_local.php';
		} else {
			require_once 'public/db_constants.php';
			require_once 'public/google_constants.php';
		}

		/* Query to fetch db version and then comparing with code version */
		try {
			$mysqlPDO = new PDO('mysql:host='.HRMS_HOST.';dbname='.HRMS_DBNAME.'',HRMS_USERNAME, HRMS_PASSWORD,array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
